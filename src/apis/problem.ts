...
// 문제 리스트 받아오기
export const getProblemList = async ({ sector, difficulty, page, limit }: GetProblemListProps) => {
...
// 문제 제출
export const postProblemAnswer = async ({ problemId, option }: PostProblemAnswerProps) => {
    const response = await axiosInstance.post<PostProblemAnswerResponse>('/api/problem/answer', {
        problemId,
        option,
    });
    return response.data;
